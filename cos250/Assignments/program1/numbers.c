...
void sixteensComplement(char characters[]);
...
char* decToHex(int inputNum){
    ...
    printf("binary Stirng before convertijng to hex: %s \n", binaries);

    char* final_hex = (char*)malloc(HEX_SIZE+3);

    char* hexVals = binToHex(binaries);

    // strcpy(final_hex, binToHex(binaries));
...
int hexToDec(char* inputStr){
    ...
    int isNegative= (inputStr[0] <= '7') ? 0 : 1;
    int len = strlen(inputStr);

    if(isNegative == 1) //if the input is negative, flip values
        sixteensComplement(inputStr);

    int sum = 0;

    for(int i = len-1, j=0; i >= 0 ; i--, j++){ //convert base 16 to base 10

        int val = (inputStr[i] < 'A') ? inputStr[i] - '0' : (int) inputStr[i] - 55;
...
/**
 * Helper function for hexToDec()
 * Performs the flipping and addition opereation for fifteens complement
 */
void sixteensComplement(char characters[]){
    ...
    for(int i = 0 ; i <  len; i++){
        ...
        characters[i] = 15 - (hextoIntMapper(c) - '0') + '0';
    }
